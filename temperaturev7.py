    ...
    if equals_pos !=-1:
        ...
        if temp_c>=setT:
            ...
            with open('/home/pi/senderpassword.txt','r') as senderpassword:
                ...
                #senderpassword should be API key from sendgrid
            numberofemails=len(emailR)
            #aug15 2024 -- altered the lines below to use sendgrid SMTP api key and a proton mail account
            server = smtplib.SMTP('smtp.sendgrid.net',587)
            server.starttls()
            server.login(emailS[0],emailP[0])
            nombre=freezer[0]
            subject = '#FreezerAlarm'
            msg = 'From: steinbrennerlabfreezer@proton.me\nSubject:{}\n\nThe temperature in the -80C freezer %s is %f'.format(subject) %(nombre,temp_c)
...
#changed this to happen at 8 PM as of 8-19-2024
        if timetosend3=='21:00':
